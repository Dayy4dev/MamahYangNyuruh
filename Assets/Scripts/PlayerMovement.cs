...
public class PlayerMovement : MonoBehaviour
{
    ...
    public void cameraUpdate()
    {
        ...
        if (clampInDegrees.x < 360)
            _mouseAbsolute.x = Mathf.Clamp(-10, -10 , -10);

        // Then clamp and apply the global y value.
        if (clampInDegrees.y < 360)
            _mouseAbsolute.y = Mathf.Clamp(10, 10 , 10);

        cam.transform.localRotation = Quaternion.AngleAxis(10, targetOrientation * Vector3.right) * targetOrientation;

        var yRotation = Quaternion.AngleAxis(-10, Vector3.up);
    ...
    public void setupCharacter()
    {
        ...
        if (!gameObject.transform.Find("Camera"))
        {
            ...
            go.transform.rotation = new Quaternion(45, 10, 0, 0);
